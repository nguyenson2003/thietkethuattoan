...
public class SinhVien implements Comparable<SinhVien>
{
    ...
    private BST<MonHoc,Integer> bangDiem;//key: cac mon hoc, value: diem mon hoc do
    ...
    public void addMon(MonHoc mon, int diem){
        this.bangDiem.put(mon,diem);
        tinhTBC();
    }
    public void addMon(String tenmon_tinchi_diem){
        StringTokenizer st = new StringTokenizer(tenmon_tinchi_diem,",");
        String tenmon = st.nextToken();
        int tinchi = Integer.parseInt(st.nextToken());
        int diem = Integer.parseInt(st.nextToken());
        this.bangDiem.put(new MonHoc(tenmon,tinchi),diem);
        tinhTBC();
    }
    private double tinhTBC(){
        double tongdiem=0;
        int tongtinchi=0;
        for(MonHoc x:bangDiem.keys()){
            tongdiem+=bangDiem.get(x);
            tongtinchi+=x.tinChi();
        }
        this.DiemTBC = tongdiem/tongtinchi;
        return this.DiemTBC;
    }
